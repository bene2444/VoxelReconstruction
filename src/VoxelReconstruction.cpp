...
void VoxelReconstruction::calibrateCameras(int argc, char** argv)
{
	for (int v = 1; v <= m_cam_views_amount; ++v)
	{
		string settingsFile = "data\\cam" + to_string(v) + "\\calibrationConfig.xml";
		calibration(settingsFile);
	}
}
